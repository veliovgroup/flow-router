...
function pathToRegExp(pathDef) {
  ...
  const pattern = pathDef.replace(/(\/)?:(\w+)(?:\(([^)]+)\)|([+*?]))?/g, (_, slash, name, customRegex, modifier) => {
    keys.push(name);
    const capture = customRegex || (modifier === '*' ? '.*' : modifier === '+' ? '.+' : '[^/]+');

    if (slash && (modifier === '?' || modifier === '*')) {
      return `(?:/(${capture}))?`;
    }
...
class MicroRouter {
  ...
  _createContext(fullPath) {
    ...
    const hash = hashIndex >= 0 ? fullPath.slice(hashIndex + 1) : '';
  ...
  _isHashOnlyChange(path) {
    const current = this._currentContext || this._createContext(this._getPath());
    const next = this._createContext(path);
    return current.pathname === next.pathname &&
      current.querystring === next.querystring &&
  ...
  _onPopState() {
    ...
    if (this._isHashOnlyChange(path)) {
      this._currentContext = this._createContext(path);
      return;
    }
